...
class SparkJob:
    def __init__(self, ts_file: str, fx_file: str):
        ...
        fx_data = sc.textFile(fx_file).map(self._parse_fx).filter(lambda x: x is not None)
        ts_data = sc.textFile(ts_file).map(self._parse_ts).filter(lambda x: x is not None)
        ...
        data_by_date = joined_data.map(lambda record: (record[0], record)) \
                                    .groupByKey() \
                                    .mapValues(list) \
                                    .collectAsMap()
